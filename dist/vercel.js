...
const init = () => {
    ...
    const deploy = async (commit) => {
        let commandArguments = [`--token=${context.VERCEL_TOKEN}`];
        ...
        if (commit) {
            const metadata = [
                ...
                `githubCommitMessage=${context.TRIM_COMMIT_MESSAGE
        ...
        const output = await exec("vercel", commandArguments, context.WORKING_DIRECTORY);
